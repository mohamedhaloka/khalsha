...
class AddEditMarineShippingServiceController extends GetxController {
  ...
  bool get shipmentSizeIsContainer => selectedShipmentSize.value == 0;
  ...
  MarineShipmentData get _marineShipmentData => MarineShipmentData(
        ...
        certificates: certificates.any((element) => element.selected.value)
            ? 'yes'
            : 'no',
        shipmentSizes: shipmentSizeIsContainer ? 'container' : 'goods',
        through: shipmentSizeIsContainer
            ? null
        ...
        certificate: certificates
            ...
            .toList(),
        containerType: shipmentSizeIsContainer
            ? containers.map((element) => element.containerType.value).toList()
            : [],
        containerCount: shipmentSizeIsContainer
            ? containers
                .map((element) => element.containerCount.value.toString())
                .toList()
            : [],
        containerContent: shipmentSizeIsContainer
            ? containers
                .map((element) => element.containerContent.text)
                .toList()
            : [],
        goodsTotalWeight: shipmentSizeIsContainer
            ? []
            : goodsTotalShipment
                .map((element) => element.totalWeight.text)
                .toList(),
        goodsOverallSize: shipmentSizeIsContainer
            ? []
            : goodsTotalShipment
                .map((element) => element.overallSize.text)
                .toList(),
        goodsUnitType: shipmentSizeIsContainer
            ? []
            : goodsUnitType
                .map((element) =>
                    element.unitType.value == 1 ? 'pallet' : 'carton')
                .toList(),
        goodsLength: shipmentSizeIsContainer
            ? []
            : goodsUnitType.map((element) => element.length.text).toList(),
        goodsHeight: shipmentSizeIsContainer
            ? []
            : goodsUnitType.map((element) => element.height.text).toList(),
        goodsWidth: shipmentSizeIsContainer
            ? []
            : goodsUnitType.map((element) => element.width.text).toList(),
        goodsWeightPerUnit:
            goodsUnitType.map((element) => element.weightPerUnit.text).toList(),
        goodsCM: shipmentSizeIsContainer
            ? []
            : goodsUnitType.map((element) => element.cm.text).toList(),
        goodsQuantity: shipmentSizeIsContainer
            ? []
            : selectedThrough.value == 0
                ? goodsTotalShipment
